    ...
    public class DatabaseReader
    {
        ...
        public Dictionary<string, Dictionary<string, string>> ReadData(string path)
        {
            ...
            using (var csv = new CsvReader(reader, config))
            {
                ...
                csv.Read();
                try { csv.ReadHeader(); }
                catch { Console.WriteLine("Чтения. Убедитесь, что разделитель выбран правильно, и файл не поврежден"); }
                try
                {
                    while (csv.Read())
                    {
                ...
                catch { Console.WriteLine("Ошибка чтения файла. Убедитесь, что категории файла указаны верно"); }
                ...
                catch { Console.WriteLine("Ошибка добавления подкатегории. Убедитесь, но номера в подкатегории не совпадают"); }
